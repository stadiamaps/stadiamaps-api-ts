...
export function instanceOfLocateEdge(value: object): boolean {
    if (!('correlatedLat' in value)) return false;
    if (!('correlatedLon' in value)) return false;
    if (!('percentAlong' in value)) return false;
    if (!('sideOfStreet' in value)) return false;
    return true;
}
