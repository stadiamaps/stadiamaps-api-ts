...
function isApproximatelyEquivalent(received: any, other: any): boolean {
    ...
    // Check if both inputs are plain objects
        ...
        for (let key of keysReceived) {
            ...
            if (_.isUndefined(valReceived) && !_.has(other, key) || _.isUndefined(valOther) && !_.has(received, key)) {
                continue;
            }
...
expect.extend({
    toBeApproximatelyEquivalent(received, other) {
        ...
        const message = pass
            ? () =>
                this.utils.matcherHint('toBeApproximatelyEquivalent', undefined, undefined, {
                    isNot: this.isNot,
                }) +
                '\n\n' +
